using Eco.Systems.Permissions.Permissions;
...
namespace Eco.Systems.Permissions.Groups
{
    public sealed class GroupsAPI
    {
        ...
        {
            var usr = chatClient as User;
            // get all the groups the user is in.
            var groups = GroupsManager.API.AllGroups().Where(grp => grp.GroupUsers.Any(sgu => sgu.Name == chatClient.Name && (sgu.SteamID == usr.SteamId || sgu.SlgID == usr.StrangeId)));
